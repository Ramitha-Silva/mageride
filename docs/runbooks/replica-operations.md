...
## 5a. The GTFS feed — a runbook of its own (Δ C126)

Loading, verifying and rolling back the national GTFS feed is
**[gtfs-day0-load.md](gtfs-day0-load.md)**. Three things from it are worth knowing while operating
this replica:

- The feed is an **externally provided file** (AL-56) and lives in the gitignored
  `infra/replica/gtfs/`. Nothing in the repository authors one.
- **`Transit__Gtfs__DownloadSigningKey` is now generated by `deploy.sh`** into `.env.replica`, and the
  stored feed zips live on the **`gtfsdata` volume** mounted at `/var/lib/mageride` in app-services.
  A rollback re-imports from the archived version's zip, so that mount is what makes a rollback
  possible after a container rebuild.
- The **day-0 Admin operator** (`GTFS_ADMIN_EMAIL` / `GTFS_ADMIN_PASSWORD` in `.env.replica`) is the
  only internal account on this replica. `. infra/replica/gtfs-lib.sh && require_token` gets a bearer
  for any admin surface, which is worth knowing before writing a second provisioning path.
