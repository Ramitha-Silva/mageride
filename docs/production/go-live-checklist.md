...
| 15a | **`www.mageride.lk` A/AAAA → the ingress load balancer.** MCS-34's public informational site (C134). The manifests, the Ingress host and the container are done and verified (`www-site`, S21/S22); what is missing is a DNS record, which is registrar work and cannot live in this repository | **OPEN** | infrastructure owner | **before the site is announced.** Nothing else depends on it — this surface is D7' §2.1's optional container and its absence costs no platform function |
| 15b | **`mageride.lk` apex A/AAAA, or an ALIAS/ANAME at the same load balancer.** The apex 301 is deployed (`mr-ingress-apex`, and the replica's HAProxy rule) but **a redirect needs something to fire from** — with no apex record the shortest and most likely address anyone types resolves to nothing. A CNAME will not do at the zone apex; the provider must offer ALIAS/ANAME or the record is an A/AAAA to the LB's address | **OPEN** | infrastructure owner | with 15a. Announcing `www.` while the apex NXDOMAINs is the failure this row exists to prevent |
| 15c | **`mr-tls-www` and `mr-tls-apex` issued and renewing.** Two certificates, not one and not a wildcard: `www.` is unauthenticated public traffic and follows the `passenger.` precedent, so the key that serves the most-linked page on the platform is not the key that serves the admin console. **The apex needs its own even though it serves no content** — the 301 is issued over TLS, so a browser reaching `https://mageride.lk` has to complete a handshake before it can be redirected, and a certificate error is what it sees otherwise | **OPEN** | infrastructure owner | with #10 (Vault/secrets) and after 15a/15b — cert-manager's DNS-01 challenge needs the records to exist |
...
| **T-5d** | DNS for `api.`, `admin.`, `fleet.`, `passenger.`, **`www.` and the bare apex** at low TTL (300 s), cert-manager issues. The apex is an A/AAAA or ALIAS — a CNAME is not valid at a zone apex — and it exists so the 301 has something to fire from (blockers 15a-15c) | `letsencrypt-prod-dns` certificates Ready, including `mr-tls-www` and `mr-tls-apex`; `curl -sI https://mageride.lk/` returns `301` to `https://www.mageride.lk/` |
