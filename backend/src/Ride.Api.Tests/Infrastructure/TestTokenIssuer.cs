...
/// </summary>
/// <remarks>
/// ride-svc is a token consumer: it holds no signing key and, in production, resolves iam-svc's
/// public half over JWKS. Standing a whole iam-svc up to get a bearer would test C020 again and
/// make this suite fail for reasons that are not ride-svc's. The harness hands the bearer handler
/// this key directly, so the token shape — the <c>sub</c>, the repeated <c>role</c> claims, the
/// <c>app</c> and <c>device_id</c> claims — matches <c>Iam.Api/Auth/AccessTokenIssuer</c> exactly.
/// C025 is where a real iam token crosses into a real ride-svc.
/// </remarks>
