...
/// </summary>
/// <remarks>
/// query-svc is a token consumer: it holds no signing key and in production resolves iam-svc's public
/// half over JWKS. Standing a whole iam-svc up to get a bearer would re-test C020 and make this suite
/// fail for reasons that are not query's. The token <em>shape</em> is what matters here — the <c>sub</c>
/// the visibility rules are applied for, and the <c>role</c> claims <see cref="Passenger"/> and
/// <see cref="Support"/> differ by.
/// </remarks>
internal sealed class TestTokenIssuer
{
    ...
    /// <summary>A back-office reader — US-24.9/24.10's read-only trips and earnings tabs.</summary>
    public string Support(Guid userId) => Issue(userId, [MageRideRoles.SupportCsr], MageRideApps.Admin);
