...
/// </summary>
/// <remarks>
/// reputation-svc is a token consumer: it holds no signing key and, in production, resolves
...
internal sealed class TestTokenIssuer
{
    ...
    public string Admin(Guid userId) => Issue(userId, [MageRideRoles.Admin], MageRideApps.Passenger);

    public string SupportCsr(Guid userId) => Issue(userId, [MageRideRoles.SupportCsr], MageRideApps.Passenger);

    public string Auditor(Guid userId) => Issue(userId, [MageRideRoles.Auditor], MageRideApps.Passenger);

    /// <summary>A driver's own token — nobody who may touch an admin route (AL-06).</summary>
