...
try
{
    scripts = MigrationEngine.LoadScripts(options.ScriptsDirectory);
}
catch (Exception ex)
{
...
var availability = await MigrationEngine.WaitForDatabaseAsync(
    options.ConnectionString, TimeSpan.FromSeconds(options.WaitSeconds), log);

if (!availability.Reachable)
{
    Console.Error.WriteLine($"error: PostgreSQL was not reachable within {options.WaitSeconds}s: "
        + $"{availability.LastError?.Message}");
    return ExitCodes.Failure;
}
