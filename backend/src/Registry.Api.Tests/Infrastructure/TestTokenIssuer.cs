...
/// </summary>
/// <remarks>
/// registry-svc is a token consumer: it holds no signing key and, in production, resolves
/// iam-svc's public half over JWKS. Standing a whole iam-svc up to get a bearer would test
/// C020 again and make this suite fail for reasons that are not registry's. The harness
/// instead hands the bearer handler this key directly, so the token shape — the <c>sub</c>,
/// the repeated <c>role</c> claims, the <c>app</c> and <c>device_id</c> claims — is the only
/// thing under test here, and it matches <c>Iam.Api/Auth/AccessTokenIssuer</c> exactly.
...
internal sealed class TestTokenIssuer
{
    ...
    /// <summary>A driver bearer for <paramref name="userId"/>.</summary>
