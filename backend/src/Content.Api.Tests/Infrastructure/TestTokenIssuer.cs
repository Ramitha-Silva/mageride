...
/// </summary>
/// <remarks>
/// content-svc is a token consumer: it holds no signing key and, in production, resolves iam-svc's
...
internal sealed class TestTokenIssuer
{
    ...
    /// <summary>A back-office admin — the only role the authoring surface admits (plus Super Admin).</summary>
    public string Admin(Guid userId) => Issue(userId, [MageRideRoles.Admin], MageRideApps.Admin);

    /// <summary>A Support CSR: an internal role with no editorial cell in URD §2.3's content row.</summary>
