...
# writes `Transit__NominatimBaseUrl` into the replica's .env.replica.
#
# ------------------------------------------------------------------------------------
...
if [ "$dry_run" = 1 ]; then
  note "would set Transit__NominatimBaseUrl=${base_url} in ${REPLICA_ENV}"
elif [ ! -f "$REPLICA_ENV" ]; then
  note "${REPLICA_ENV} does not exist yet. After infra/replica/deploy.sh has run, add:
      Transit__NominatimBaseUrl=${base_url}"
elif grep -q '^Transit__NominatimBaseUrl=' "$REPLICA_ENV"; then
  sed -i "s|^Transit__NominatimBaseUrl=.*|Transit__NominatimBaseUrl=${base_url}|" "$REPLICA_ENV"
  ok "updated Transit__NominatimBaseUrl in ${REPLICA_ENV}"
else
  {
    echo
    echo "# --- generated by deploy-nominatim.sh: the self-hosted geocoder on its own VPS ---"
    echo "Transit__NominatimBaseUrl=${base_url}"
  } >> "$REPLICA_ENV"
  ok "added Transit__NominatimBaseUrl=${base_url} to ${REPLICA_ENV}"
fi
