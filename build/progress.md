...
- Total components: **133** · estimated sessions: **270**
...
| 3 | C045–C066 (23) | every business service test suite green; ledger balances in all money tests |
...
- **Component:** C063 admin-bff-verification — 2026-08-01
- **Status:** DONE — `dotnet test backend/src/AdminBff.Tests -c Release --filter Category=Verification`
  is **17/17 green** and the whole suite is **91/91** (was 65);
  `dotnet build backend/MageRide.sln -c Release` is 0 warnings 0 errors;
  `bash infra/scripts/migrate-verify.sh` is **446/446**; Spectral is 0 errors.
- **Notes:**
  **What was built —** the Verification Officer's whole surface (SCR-AP-003/003a/003b/003c): three
  queue feeds with search and a status filter, one subject-agnostic detail carrying the flagged
  fields, the attached documents and SCR-AP-003a's per-step decision rail, the fleet-org detail with
  its AL-49 payout evidence, `PUT …/fields/{key}` (confirm, or edit and confirm), `POST …/approve`
  and `…/reject`, and `GET /v1/admin/documents/{docId}` — the audited viewer. Mapped on C062's
  group, so it inherited the AL-06 gate, the D-35 interceptor, the surface marker and the RBAC
  test's coverage; nine probe rows were added to `RbacMatrixTests.Cases`, which is what that test
  demands rather than merely permits.

  **The two decisions this component is actually about.**

  • **A queue is a query over `registry.document_fields.verify_status = 'pending'`, and that *is*
  AL-27's fence.** The prompt states it as a rule to obey ("the officer sees only PENDING items");
  making membership *be* that predicate means an auto-verified document cannot appear rather than
  being filtered out by code that could later stop filtering. The partial index
  `ix_document_fields_pending` (0305) exists to be this query's index, and **the same count is
  US-2.10a's approval gate** — one source, so the queue and the refusal can never disagree about
  whether a submission is settled. What a row's `status` carries is therefore the *subject's* own
  registration status, not the queue's: every member is by construction awaiting review, and what
  the officer needs to know is whether this is a renewal on an already-approved applicant or a
  resubmission after a refusal. That is what D2's status filter filters on.

  • **Every document fetch goes through the audited route.** AL-39 asks for two things at once —
  short-lived signed object-storage URLs **and** a `DOC_VIEW` row per read — and the obvious reading
  (put a pre-signed bucket URL in `thumbUrl`) satisfies the first while silently breaking the
  second: the officer's browser would fetch somebody's licence straight from storage and nothing
  would record it. So `thumbUrl`/`fullUrl` are `/v1/admin/documents/{docId}?variant=…`, and *that*
  route records the view and then mints the signed URL it `302`s to. The signed URL is where it
  always was; what changed is that it is minted per view, on the way out of the row that records the
  view. **One view is one row** — a grid of four thumbnails is four looks at four documents. The
  cost is one C062 change: the D-35 interceptor's success window is now 2xx **and** 3xx, because
  treating a redirect as a failure would make the one audited read on this surface the one that
  records nothing.

  **Who writes what, and why this is not a second writer.** The BFF rule held: the AL-30 recompute
  is registry-svc's (it built `/v1/internal/vehicles/{id}/onboarding/recompute` **for this caller**
  and its own file says so), and AL-50's fleet-vehicle gate plus AL-49's organisation approval are
  fleet-svc's (its whole `/v1/internal/fleets/**` plane exists for this BFF). What is written here
  is what nobody exposes a route for: `registry.vehicles.rejection_reason` — registry-svc's file
  leaves it to this component **by name** — `registry.driver_profiles.verified_at` and its new
  `rejection_reason`, and the confirmations on `registry.document_fields`. Two new upstreams
  (`Registry`, `Fleet`), both `/v1/internal` planes, both taking the shared key with the officer's
  id on the body — C052/C053's split exactly.

  **Ordering that is load-bearing —** the field, the audit row and the commit go together, and the
  recompute follows the commit because registry-svc has to read what that transaction wrote. An
  unreachable registry-svc is therefore a `503` on a request whose field is already confirmed; both
  halves are idempotent so the retry is clean, and the alternative — swallowing it — is precisely
  the gap that route was built to close (a vehicle left at `pending_review` for a question the
  officer already answered).

  **Four smaller rules, each with a failure behind it —**
  • **A field key is decided on every row that carries it.** A licence is two documents; confirming
  one row leaves the subject unapprovable for a field the officer believes they cleared.
  • **An edit reaches an unflagged field; a bare confirm does not.** AL-29's premise is that OCR can
  be confidently *wrong*, so correcting an `auto_verified` value is legitimate — while confirming
  one nobody flagged is a decision with no question in front of it, and a double click must change
  nothing. An edited value becomes `manual` with `confidence` NULL, which is what
  `ck_document_fields_manual_confidence` demands and what stops an invented score reading as
  evidence.
  • **Withdrawing a rejection is its own fact.** registry-svc declines to auto-approve a REJECTED
  vehicle ("a decision that four green steps do not overturn"), so approving a resubmission has to
  reopen it first. `VERIFICATION_REOPENED` is recorded separately, which keeps the trail honest when
  the approval that follows is then refused by AL-10 — and the reopen answers `409
  registration-exists` when the plate the rejection freed has since been claimed (D-37), because
  PENDING is inside `ux_vehicles_regno_active`'s predicate.
  ...
  driver is one synthetic `profile` step, because a decision rail with nothing in it reads as
  "nothing to check".

  **Spec gaps / micro-change-sets raised —**
  1. ⚠ **A rejected driver had nowhere to put the reason.** Migration **0315** adds
  `registry.driver_profiles.rejection_reason`. AL-39 makes the family subject-agnostic and US-2.15
  makes the reason mandatory and "surfaced verbatim to the applicant"; `registry.vehicles` (0303)
  and `registry.fleets` (0301) both have the column and `registry.driver_profiles` (0304) had
  `verified_at` and nothing else, so an officer's refusal of a licence could be recorded in
  `audit.events` and never shown to the driver it was about. **server_db_schema.md §2 / D4' §2
  should carry it.** No companion timestamp and no `status` column: the state is derived exactly as
  `onboarding_status` is.
  2. ⚠ **SCR-AP-003's search and status filter had no parameters.** D2 §SCR-AP-003 requires "search +
  status filter" on all three tabs and D3's AL-39 block declares neither. Added to `admin-bff.yaml`
  as `VerificationSearch` / `VerificationStatus`. **D3' Δ 2026-06-28 item 8 should carry them.**
  3. ⚠ **`VerificationDetail` had no per-step breakdown.** SCR-AP-003a's decision rail is "per-step
  breakdown; Approve unlocks when all confirmed" and the AL-39 response shape is
  `{subject, fields, documents}`. Added `steps` + a `VerificationStep` schema. **D3' should carry
  it.**
  4. ⚠ **D-11's merchant bind has no merchant id anywhere on this build map.** `approve` is specified
  to trigger `POST /v1/internal/vehicles/{id}/merchant`, which **requires** a `merchantId`; no
  component produces one. `merchantBound` answers `false`, the approval logs the fact, and fare-svc
  keeps answering `402 merchant-not-onboarded`. **An OnePay merchant-onboarding path is missing from
  the build map**, not just from this component.

  **Contract changes (`admin-bff.yaml`, 8) —** `search` + `status` on the three queues;
  `VerificationDetail.steps` + the `VerificationStep` schema; `DocumentRef`'s links respelled as the
  audited viewer with the reason; `variant` on `GET /v1/admin/documents/{docId}`;
  `OrgQueueRow.kycStatus` given an enum and a definition (`complete` once the org carries a business
  registration and a contact number — the payout profile's own state is the separate
  `payoutProfileStatus`, added); `OrgQueueRow.status` lost `SUSPENDED`, matching C058's `FleetStatus`
  and the three-value CHECK; `registration-exists` added to approve's `x-error-codes`. **No new
  error code** — every refusal maps onto one the kernel already has.

  **A C062 behaviour changed (one line, argued) —** `AuditInterceptor`'s success window is
  `2xx..3xx` rather than `2xx`. Nothing else on the surface answers a 3xx today; the GTFS
  pass-through sets its own status and records before it streams, so it is unaffected.

  **Testing decision worth knowing —** `StubUpstream` gained two planes that **write**, unlike the
  four that only answer. "Approving a fleet org's payout profile makes `payTo` available to
  subscription-svc" is a claim about `registry.fleet_payout_profiles`, and asserting it against a
  canned `{"status":"verified"}` would assert nothing — so the stubbed registry recompute applies
  AL-30's rule and the stubbed fleet approval does AL-49's supersede-then-verify in the order
  `ux_payout_profile_verified` demands, both against the same Postgres. The test then reads the
  table with subscription-svc's own predicate. AL-10's mandatory-document gate is deliberately not
  modelled: registry-svc's suite proves it and admin-bff only ever reads its outcome.
  `AdminBffHarness`'s client no longer follows redirects, because the 302's `Location` is the
  assertion.

  **For C064 / C065 —** the pattern is unchanged and now has a second worked example: add a
  `Map…Endpoints(this IEndpointRouteBuilder admin)`, gate with `RequireFeature`, record with
  `IAdminAuditContext.Record(...)` and `FlushAsync(unitOfWork, ct)` inside your own transaction, and
  **add a probe row to `RbacMatrixTests.Cases` or the route-table test fails**. C064's vehicle
  directory (AL-42) wants document thumbnails "→ the AL-39 viewer": use `IDocumentLinks.Create(docId,
  variant)` and do **not** mint a second link shape — the viewer is what writes `DOC_VIEW`, and
  AL-40's `PII_READ` is recorded the same way (`.Audited(...)` on a GET, handler calls `Record`).
  `IVerificationRepository.FindDocumentAsync` resolves a doc id out of either `registry.documents`
  or `docs.uploads`. **A Dapper trap this component hit twice:** `DefaultTypeMap.FindConstructor`
  matches a record's constructor **positionally and by reader field type**, so a projection whose
  column order differs from the constructor fails at *run time*, and Npgsql reports `text[]` as
  `System.Array` — an aggregated `string[]` must come back through a settable property, not a
  constructor parameter.

  **Build host —** Docker for one Testcontainers fixture (Postgres only). The full `AdminBff.Tests`
  suite takes ~2 min including the container start and migration; `--filter Category=Verification`
  ~34 s. `migrate-verify.sh` was re-run for 0315 and is 446/446. The replica stayed down throughout.
  **No new NuGet reference and no new PostgreSQL extension.**

---

- **Change set:** ADD v3.6 — payment custody (AL-57 / AL-58 / AL-59) — 2026-08-01
- **Status:** SPECS + CONTRACTS + MANIFEST DONE — no service code changed yet.
  Spectral 0 errors; `dotnet build backend/MageRide.sln -c Release` 0 warnings;
  `build/tools/generate_build_plan.py` re-run (133 components, 202 screens mapped).
- **Notes:**
  **The vendor fact.** OnePay supports **one merchant account per merchant** and does not create
  per-driver merchant sub-accounts. D-11 assumed it did, so `registry.driver_payouts`,
  `POST /v1/internal/vehicles/{id}/merchant`, `Onepay__MerchantOnboardingUrl` and
  `402 merchant-not-onboarded` could only ever hold or check MageRide's own id — the same value for
  every driver. **D-11 is retired, not completed.**

  **The decision taken (user, 2026-08-01).** Card acceptance is kept. A card passenger tops up a
  **passenger wallet** and pays with the new `wallet` method; **MageRide takes custody** of that
  money and discharges it through a **new automated weekly payout rail**. LankaQR ride payments
  become the **driver's own** bank QR. The rule the three changes reduce to: *OnePay and the
  platform's LankaQR merchant are used only where MageRide is the **payee**; where the payee is a
  driver or a fleet owner, the rail is the payee's own bank instrument.*

  **Two things found while writing it, neither of which was the reported gap.**
  • **`billing.accounts.owner_type` had no `passenger` value** — `driver`/`fleet`/`platform`/
  `suspense` only. The "wallet" this platform has built is the *driver's*, for daily fees. A
  passenger wallet has no ledger existence, so AL-57 is a new account type plus a passenger-facing
  top-up and balance surface, not a reuse of something already there.
  • **The `lankaqr` ride rail already collected into a platform merchant** (`Fare:LankaQrMerchantId`)
  and credited the driver nothing but a `fares.driver_earnings` read-model row. That is a second,
  undetected custody path **live since C050** and predating this decision. AL-59 closes it.

  **Payout policy, as instructed:** weekly, **full sweep, no minimum, no holdback**. The driver's
  existing wallet is the fare accumulation account — fares credit it, the D-08 daily fee debits it,
  payouts sweep it. `Payout:RetainMinor` exists and **defaults to 0**.
  ⚠ **Named consequence (D5 §8.1, accepted as a risk):** `WalletGate.Decide` has two refusal
  branches, not one. `unconfirmable` is the billing-outage case; **`insufficient` is a normal-state
  balance check** — on the 2nd trip of a Colombo day with the fee unpaid and the wallet at zero the
  driver is dropped from the candidate set. Cash and driver-QR fares never credit the wallet, so
  after a sweep those drivers get their free first trip and are refused every trip after it until
  they top up. The remedy is `RetainMinor = one daily fee`; it is a setting, not a redesign.
  ⚠ **Licensing precondition, unresolved:** custody of passenger balances + outward bank transfers
  makes MageRide a payment intermediary — CBSL authorisation under the Payment and Settlement
  Systems Act, and a sponsor bank for LankaPay/CEFTS origination. **A go-live gate, not an
  engineering task.** No component discharges it.

  **Specs changed —** ADD §1.18 (new), §11.9 **replaced** by the passenger-wallet flow, §11.9a
  (new) for the payout run, §6 (`payout-svc` added; registry/wallet/fare rows rewritten), §9.1
  vocabularies, the D-11 decision row and the implementation checklist. D4 + `server_db_schema.md`:
  `registry.driver_payouts` → `registry.driver_payout_profiles`, plus `billing.payout_batches` /
  `billing.payouts`, `owner_type` +`passenger`, journal kind +`driver_payout`. D3: the `/fare/pay`
  method enum, the merchant route struck out for `GET·PUT /v1/drivers/payout-profile`, a whole
  `payout-svc` route table, Mode B loses `onepay`. D5 §8.1: the `wallet` transition, the AL-47 QR
  arm the diagram was missing, and the surcharge removed with the rail it recovered. D7 §4.2 and
  D2 (SCR-DA-022a new; both pay sheets).

  **Contracts changed (5 + the index + 1 new) —** `fare.yaml` (methods, both ride-gateway callbacks
  removed, `PaymentInitiation` reshaped, `ProviderCallback` deleted), `registry.yaml` (merchant
  route out, three payout-profile routes + `DriverPayoutProfile` in), `wallet.yaml`,
  `subscription.yaml` (`onepay` out of Mode B — **kept** on bulk-voucher purchase, where a driver
  pays MageRide and one merchant account is exactly right), `_shared.yaml`, the directory's
  `CLAUDE.md`, and **new `payout.yaml`**. `admin-bff.yaml` is untouched by this change set — the
  verification surface does not name a payment rail. `payout-profile-not-verified` is **reused** from C058 rather than duplicated.
  `merchant-not-onboarded` deliberately **stays** in the enum with a retirement comment: it mirrors
  `MageRideErrors` one-for-one and fare-svc still references it — both come out in the same change.

  **Manifest —** C028 (D-11 deliverable out, payout profile in), C046 (passenger wallet, the two
  internal ledger rails), C048 (`onepay` fence), C050 (rails rewritten, 6 new DoD items), the app
  components' pay sheets, and **new C133 `payout-svc`** (wave 3, after C046/C028, 3 sessions).
  **Regenerating `progress.md` overwrites the Status column, the Session Handoffs log and any
  hand-written wave-gate annotation** — all three were spliced back from the pre-run copy and the
  diff verified down to the three intended lines. Do the same next time.

  **Migrations landed 2026-08-01 — 0316, 1007, 1109, 1313; `migrate-verify.sh` 461/461** (was 446;
  15 new checks, 2 count assertions and 2 enum assertions revised). **Every one is additive**, and
  that is the whole sequencing decision: `registry.driver_payouts` is NOT dropped and
  `fares.ride_payments.method` still admits `onepay`/`lankaqr`, because registry-svc and fare-svc
  are DONE components that still name both. DbUp applies this directory to the replica, so a drop
  here would take two running services down between the migration and the code that stops
  referencing them. **The drop ships with C028** — corrected 2026-08-01 during C050: registry-svc
  (`MerchantService`, `SubscriptionRepository`, `POST /v1/internal/vehicles/{id}/merchant`) is the
  *last* holder of the reference, not fare-svc, and the manifest already lists the removal among
  C028's deliverables. 0316's own header names C050 and is left alone: an applied script is
  immutable, and this record is where the correction belongs. The contract enum is already narrow while the CHECK is
  not, deliberately: the contract states the intent and the database moves last.
  Two ordering facts worth keeping: `docs.uploads` is C005's (1301) and the 03xx registry range
  runs first, so 0316 declares the two upload columns bare and **1313** closes the FKs — C003's own
  arrangement for `registry.fleet_payout_profiles`; and `billing.payouts.journal_entry_id` is
  `NOT NULL` so the schema cannot hold half of the debit/instruction pair.

  **Next, in order:** **C046 wallet-svc** — passenger accounts on the existing ledger, plus two
  rails on the internal seam that already carries `/{driverId}/debit|credit` and
  `/fleet/{fleetId}/debit|credit`: `trip-payment` (passenger wallet → driver wallet as ONE balanced
...
- **Component:** C050 fare-svc-payments (AL-57/AL-59 rails) — 2026-08-01
- **Status:** DONE — `dotnet test backend/src/Fare.Api.Tests -c Release` is **112/112** (was 113:
  3 removed, 2 added); `dotnet build backend/MageRide.sln -c Release` 0 warnings; Spectral 0 errors.
- **Notes:**
  **What changed —** `POST /v1/fare/pay` now takes `cash | wallet | scan_driver_qr`. The `wallet`
  rail calls wallet-svc's `/trip-payment`; `onepay` and platform-merchant `lankaqr` are gone, and
  with them the two provider-callback routes, both gateway implementations, the D-11 merchant lookup
  and `402 merchant-not-onboarded`, the +5 % surcharge, and seven `Fare:*` configuration keys.

  **The ordering inversion is the thing to understand.** Everywhere else on this surface the outward
  hop *follows* the decision — the fare is still correct if wallet-svc is down and the D-05 debt
  waits for the next trip. On the `wallet` rail the hop **is** the decision: whether the fare can be
  paid at all is a question only the ledger can answer. So the money moves first and the state moves
  second. The call is idempotent on `trip_payment:{ridePaymentId}`, so the one bad interleaving —
  ledger moved, process died before transitioning — is repaired by the passenger tapping Pay again.
  An unreachable wallet **refuses** rather than settling: reporting `Succeeded` because the ledger
  could not be reached would tell a driver they had been paid a fare that never moved.

  **`Failed`, `Retried`, `Pending` and `Overpaid` are now unreachable, and stay in the machine.** All
  four were only ever reached by a provider callback. `PaymentStateMachineTests` still asserts them
  against D5' §8.1 and historical rows carry them — but no route can produce one, so **three
  integration tests were removed rather than rewritten** (duplicate-callback dedupe, the late-success
  overpayment, the retry chain). A test that drove them would be testing its own fixture. Two tests
  were added for the new rail: settles on the spot through one ledger call, and refuses a short
  balance while moving nothing.

  **Two corrections to the change set, both found by implementing it.**
  1. ⚠ **The `registry.driver_payouts` drop does NOT ship here.** 0316's header and the change-set
  record both named C050; the *last* holder of the reference is registry-svc (`MerchantService`,
  `SubscriptionRepository`, `POST /v1/internal/vehicles/{id}/merchant`), which is **C028**. Migration
  1008 was written, caught by a grep before it ran, and deleted. 0316 is left alone — an applied
  script is immutable — and the record above carries the correction.
  2. ⚠ **`ck_ride_payments_method` is NOT narrowed, and should not be.** Postgres applies a narrowed
  CHECK to existing rows at ALTER time, so removing `onepay`/`lankaqr` would either fail the
  migration on any database with history or force a rewrite that misstates how somebody actually
  paid. A row saying somebody paid by OnePay in July is a **fact**. The fence is held four times over
  by things a CHECK cannot express anyway — no value in `PayableMethods`, none in `fare.yaml`'s enum,
  no gateway implementation, no callback route. **A CHECK cannot say "nothing writes this any more";
  the absence of the code says it.**

  **P-04 restated, not changed —** cash and driver-QR are paid by the rider, the wallet by the
  booker. The rule was always about whose *instrument* settles it: the person in the car hands over
  cash or scans a QR, and a stored balance belongs to whoever booked and topped it up.
...
- **Component:** C065 admin-bff-finance-pdpa — 2026-08-01
- **Status:** DONE — `dotnet test backend/src/AdminBff.Tests -c Release --filter Category=FinancePdpa`
  is **27/27 green** and the whole suite is **162/162** (was 122). All four DoD items have a named
  test asserting against the database rather than a response body, because all four are claims about
  state on the far side of a decision: *a reversal posts a balanced journal entry and appears in the
  driver's ledger and the audit log* (`A_reversal_posts_a_balanced_entry_reaches_the_ledger_and_is_audited`
  — Σ of the entry's legs is 0, a `billing.wallet_transactions` line exists, and one
  `WALLET_FEE_REVERSED` row names the actor); *an export produces a downloadable ZIP and expires its
  signed URL* (`An_export_is_fulfilled_as_a_readable_zip_behind_an_expiring_link` — the archive is
  read back off the store and opened, its manifest names the subject, and the link is re-minted on
  every read); *an erasure honours the statutory hold list and leaves the audit subset intact*
  (`An_erasure_anonymises_the_identity_and_retains_what_a_statute_requires` plus
  `An_erasure_held_by_a_live_ride_is_refused_and_changes_nothing`); *reconciliation flags a gateway
  settlement mismatch into the exception queue* (`A_settlement_mismatch_reaches_the_exception_queue`,
  which seeds a ledger that disagrees with a gateway session). `migrate-verify.sh` **469/469**,
  Spectral **0 errors**, `dotnet build backend/MageRide.sln -c Release` **0 warnings**.
- **Notes:**
  **The one structural decision: this surface moves money and writes none of it.** Every mutation it
  offers is forwarded to the service that owns the rows — wallet-svc posts the reversal (it is the
  only writer of `billing.journal_postings`, and its own file names admin-bff as the caller entitled
  to `kind='adjustment'`), fare-svc raises the refund and its balanced entry and calls the gateway,
  reputation-svc resolves a fraud flag. `FinanceRepository` has no `INSERT` and no `UPDATE` at all,
  and `Only_two_finance_routes_mutate_and_both_forward` enumerates the route table so a later
  component cannot hang a ledger write off a finance path. What C065 contributes is the RBAC gate,
  the queue the decision is made from, and the D-35 row saying a human made it — two rows for one
  action, which is this platform's documented shape.

  **Four URD §2.3 rows on one screen, and the obvious single row is wrong three times.**
  Reconciliation and the transactions report are **Finance** (CSR and Verification Officer ➖). The
  reversal is **Driver wallet adjustments / reversals**, the one row that exists for this button and
  whose cells are ✅ for exactly Super Admin and Finance — so the prompt's fence "Finance/Super-Admin
  only" is not a role list written into a route, it is what the matrix already says, and there is no
  ◐ in the row to fence. Refunds are **Refunds**, whose CSR cell `◐ raise/recommend` opens the queue
  and withholds the button with no platform-wide fence at all, because `PermissionCell.Parse`
  already trades Write for Raise. **One deviation from the prompt's fence, and it is deliberate:**
  URD §2.3 gives **Admin ✅ on Refunds** and 👁 on the reversal row, so an Admin may execute a refund
  and may not reverse a fee. AL-06 makes §2.3 the source of truth for RBAC and the fence's wording
  matches the *wallet-reversal* row exactly; the reversal route honours it to the letter and the
  refund route follows the matrix. `Only_finance_and_super_admin_may_reverse_a_fee` and
  `A_csr_sees_the_refund_queue_and_cannot_execute_one` assert both cells by name.
  ...
  **The two review queues are gated where they belong, not where they were built.** ADD §6 lists the
  ...
  — but an expiring insurance certificate is a document review (**Verification**; E-03's expiry is
  what AL-10's gate turns on) and a confirmed E-07 signal leads to a suspension (**Moderation**).
  Gating either on Finance would hand the screen to the one role with no use for it and take it from
  the two that have.

  **The fraud queue reads `reputation.fraud_flags` and does not decide.** reputation-svc owns the
  table and exposes both the list and the `/resolve`, and the gateway routes both there at Order 20 —
  but it holds no identity, so its queue is a list of bare UUIDs nobody can work. The rows are joined
  to names here, exactly as C064's three directories join five other services' tables, and
  `resolveUrl` points the decision back at reputation-svc. A test asserts **no call reaches
  reputation-svc from this component at all**.

  **Four spec gaps found, all resolved in migrations and all worth recording as micro-change-sets.**
  (i) **`iam.users` had no column an erasure could write** (`0110 anonymised_at`). E-06's erasure is a
  *soft* anonymisation — the row survives so every ride, posting and audit event referencing it still
  resolves — and nothing in §1 recorded that it had happened, so `admin-bff.yaml`'s
  `PassengerRow.status = deleted` was a value nothing could produce (the C064 handoff says exactly
  that and leaves the enum for this component) and a second erasure request would re-anonymise an
  account and report it as fresh work. **D4' §1 should carry the column.** Not `is_blocked`: blocking
  is a moderation decision an admin can undo.
  (ii) **`pdpa.requests` had nowhere to put a refusal's reason or its decider** (`1314`). §16 gives
  the table a `Rejected` status and D3' a `/reject` route whose body the contract types as a
  **required** reason. `hold_reason` is not it — `ck_pdpa_requests_hold` ties that column to
  `FulfilledHold`, the opposite outcome, and one field carrying both would leave the SLA queue unable
  to tell a partial fulfilment from a denial. `decided_by` is the same gap on the other axis: D-35's
  row records who called the route, and neither the subject's status read nor the queue can join
  `audit.events`. **§16 and D4' §11-16 should carry both.**
  (iii) **The R-19 Overpaid queue had no index** (`1008`). 1002's three all start from a ride, a QR
  claim or the retry chain; "everything currently Overpaid, oldest first" is the refund queue's whole
  second half and would have scanned every payment attempt on the platform.
  (iv) **Gateway settlement had no per-rail-per-day index** (`1111`). 1107's three all start from one
  session; SCR-AP-006 starts from a rail and a window.

  **One spec conflict resolved: `/v1/pdpa` versus AL-02.** D3' heads the family "pdpa-svc (via
  admin-bff) — data rights (`/v1/pdpa`)" and marks all three routes Bearer; `gateway-routes.json`
  already sends the prefix to this cluster at Order 90; iam-svc's `DELETE /v1/users/me` answers 202
  with `Location: /v1/pdpa/{requestId}`, a URL a passenger's app follows. AL-02 says admin-bff serves
  nothing driver- or passenger-facing, which C062 held as "every route is under `/v1/admin`". Read as
  a rule about a **console**, the two agree: three routes by which a person exercises a statutory
  right over their own record are not a console. The start-up guard now names `/v1/pdpa` explicitly
  rather than being loosened, so a *fourth* prefix still refuses to start, and
  `The_data_subject_routes_are_reachable_by_an_end_user_and_are_the_only_ones_that_are` asserts the
  route table holds exactly those three. The subject routes are authenticated-and-own-scope rather
  than matrix-gated, because §2.3's End-user-account-management row gives PAX and DRV ➖ — correctly,
  since it is about operating on *other people's* accounts — and gating an own-account right on it
  would refuse every data subject their own data. **A request that is not yours is a 404**, never a
  403: the difference would make the route an oracle over other people's live erasure requests.

  **The erasure's hold list has two kinds of entry, and conflating them is the mistake the design is
  about.** A *blocking* hold (an in-flight ride or session, an open dispute, an unsettled payment or
  refund, money still in the wallet) is a live operation that anonymising would break: it answers
  **409** naming what is holding it, and it lifts on its own. A *retention* hold (`billing`
  postings, `audit.events`) is a record a statute requires be kept and is what turns `Fulfilled` into
  `FulfilledHold`. Treating them alike would either refuse every erasure for ever — every account has
  a ledger — or anonymise somebody who is in a car right now. **`outcome` is optional on the fulfil
  body and the derivation wins over `Fulfilled`**: anything retained under a statute *is* a
  `FulfilledHold` whatever the button said.
  ...
  **`phone` becomes NULL and `email` becomes a per-account `.invalid` address, and neither is
  ...
  survives it. `registry.driver_profiles.display_name` is emptied rather than nulled because the
  column is `NOT NULL` (0304). Also cleared: `iam.emergency_contacts`, `iam.saved_addresses`,
  `iam.phone_lookups` (the keyed digest of the number the anonymisation just removed) and every live
  `iam.sessions` row.

  **Two of the three `rides` columns the C032 handoff named for this component are deliberately not
  erased, and that is stated rather than done quietly.** `rides.rides.rider_phone_hash` **is** cleared
  — but only where the subject is the *rider*, since where they are the booker it is somebody else's
  number and `ck_rides_proxy` requires one of the two columns to survive.
  `rides.rides.recipient_phone` is a **third party's** number on a delivery the subject booked: it is
  the recipient's data, and `ck_rides_package_recipient` makes it `NOT NULL` for a package ride, so
  clearing it would break the record of a delivery that happened. `rides.proof_artifacts.storage_url`
  points at bytes in the D-36 bucket under NFR-28's own `ephemeral/` lifecycle rule, which deletes
  them on a deadline this service does not own. **Both belong to ride-svc and want a decision from
  whoever owns E-06's scope, not a quiet write from a BFF.**

  **The exception queue's four classes are derived and never stored, and that is forced.** wallet-svc
  refuses a callback whose amount disagrees with its session, logs the numbers and leaves the session
  `Pending` (`TopupService.SettleAsync`) — there is no exception column, and adding one would give
  this component a write on another service's table. So `amount-mismatch` / `settled-not-posted` /
  `unsettled` / `gateway-failed` are a `CASE` over the state, the ledger and the clock, and a session
  that resolves itself leaves the queue with nobody having to close it. **A lost callback and a
  refused mismatch are both `unsettled`** because the schema genuinely cannot tell them apart, and the
  operator is told that rather than shown a guess. `AdminBff:Finance:SettlementGracePeriod` is
  **one hour and deliberately not D6' §7.1's 90 seconds**: that window is how long a client polls, and
  a session open a minute later is somebody typing a card number.

  **Decisions that are load-bearing and cheap to get wrong later.** The reversal's ledger key is
  `adjustment:fee_reversal:{driverId}:{vehicleId}:{feeDate}` — the business fact, so a double click
  replays instead of crediting twice and this route needs no command log. That means **one reversal
  per charge, ever**, which is the right bound; **the audit row is written on the replay too**,
  because what D-35 records is that an operator performed the action. The refund queue is a **union**
  of `fares.refunds` and R-19's *unraised* `Overpaid` payments, because a list of raised refunds would
  hide exactly the failure §11.14 exists to catch. The transactions report reads
  `billing.journal_entries`, **not `billing.wallet_transactions`**: the projection is one row per
  account leg, so a driver-to-driver transfer appears twice and a report that summed it would double
  the platform's transfer volume; the two parties are the two legs named by sign, one projection for
  all four kinds. The JSON, the CSV and the PDF are **one query**, so "the export matches the screen"
  is structural.

  **The PDF is written by hand, in ~200 lines and with no dependency.** wallet-svc answers `415` to
  `application/pdf` on its statement route and its reasoning is right about a *styled statement* and
  wrong about a *table*: one of the base-14 fonts every conforming reader must have needs no embedded
  font, so the alternative was adding a rendering library to `Directory.Packages.props` for one
  back-office download. The cross-reference offsets are **measured, not computed** — a file whose xref
  is one byte out opens in a forgiving reader and fails in a strict one — and a test walks
  `startxref` back to object 1 to prove it. **It must not be extended to anything trilingual**:
  WinAnsi has no Sinhala or Tamil glyphs and faking them would be worse than the 415 (D-26).

  **The export archive is `ephemeral`, which is a decision and not a default.** Any non-null
  `Retention` puts the object under the prefix D-36's one lifecycle rule matches — right, because an
  export is a copy of everything the platform holds about one person and must not become a permanent
  second copy. It is emphatically not `Retained`, which is for objects the platform keeps *serving*.
  **The images behind `documents.json` are named and not enclosed**: an archive assembled in one
  request cannot carry a folder of scans, and a download that timed out would be a fulfilment that
  did not happen — the ZIP's `README` says they are available through support.

  **`Pdpa__DueDays` finally lands, and it reports rather than decides.** D7' §4.2's sixth admin-bff
  variable is now `AdminBff:Pdpa:DueDays`; the deadline itself is `pdpa.requests.due_by`'s column
  default (1306), which iam-svc also lets stand — so the two services that open a request agree about
  a statutory clock without either owning it.

  **For C103/C104/C105 (the Admin Portal) —** `AdminMenu` gained four entries
  (`document-expiry`, `fraud-review`, `transactions`, `refunds`) and every one is gated on the same
  (area, capability) pair as its endpoint, so a nav item cannot promise a screen the API refuses. The
  hold codes on a PDPA request are **machine codes, never sentences** (D-26): the portal and the apps
  own the Si/Ta/En bundles for `active-ride`, `open-dispute`, `unsettled-payment`, `wallet-balance`,
  `financial-records` and `audit-trail`. `FraudFlagRow.resolveUrl` is reputation-svc's route and the
  button must post there, not here.

  **For C118 (contract tests) —** `admin-bff.yaml` gained **10 operations** (reconciliation and its
  exceptions, the refund queue and its POST, three transactions renderings, the document-expiry and
  fraud queues, and the PDPA admin queue) and **13 schemas**; `reverseDriverFee`'s 200 gained
  `balanceAfterMinor` and `replayed`, `PdpaRequest` gained six fields, and `fulfillPdpaRequest`'s body
  is now optional.

  **Build host —** the replica stayed down throughout. `AdminBff.Tests` takes ~5 min 20 s for the
