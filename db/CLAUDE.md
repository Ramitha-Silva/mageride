...
- **A service with idempotent POSTs owns its own `command_log`** (`iam` 0104, `registry` 0307,
  `dispatch` 0710, `reputation` 0803), shaped like `rides.command_log` (0603) minus the
  aggregate-id column. D4' §5 prints DDL for `rides` only; pointing a second bounded context at
  that table would give two services one shared primary key, so a registration and a ride could
  collide on an identical client-generated `Idempotency-Key`. All four are raised as
  micro-change-sets in `build/progress.md`.
