...
|---|---|---|---|---|
| PUT `/v1/drivers/profile` | Bearer (driver) | [ADAPT] | **multipart or JSON w/ uploaded file IDs** (Δ MCS-01) — `{driverName, profilePhotoFileId, licenseFrontFileId, licenseBackFileId, nicNo?, allowedVehicleTypes?[]}` → `{driverId, status, fields:[{key,value,source,verifyStatus}]}` | **Profile Setup** (SCR-DA/DI-003a): writes registry.driver_profiles + registry.documents(kind=driving_license, vehicle-less); queues DL OCR which extracts **licenceNo, expiry, nicNo, allowedVehicleTypes** (AL-29). Any **driver-supplied** `nicNo`/`allowedVehicleTypes` (unclear scan) is stored `source='manual'` + `verifyStatus='pending'` → Verification-Officer queue (US-2.4a). **Profile photo required.** Precedes Home; no vehicle needed (Change 6/22). **Δ MCS-02: the multipart arm also carries `licenceNo` + `licenceExpiry`** — SCR-DA/DI-003a draws a ✎ on both and only `nicNo`/`allowedVehicleTypes` could be sent, so an edit to either was silently dropped (the C068 handoff's open finding). **Δ MCS-01: the multipart arm** carries `photo`, `licenseFront`, `licenseBack` and a per-part `…CapturedVia` (AL-43, `camera_dragcrop|gallery`) → `docs.uploads`, so the three ids the JSON arm needs can be minted; before it, no route on the platform created a `docs.uploads` row for an onboarding document and this screen could not be completed |
| GET `/v1/drivers/profile` | Bearer (driver) | [NEW] | `{driverId, status, displayName, photoUrl?, nicNo?, allowedVehicleTypes[]}` **or `null`** | **Δ MCS-05.** Has this driver completed Profile Setup? SCR-DA/DI-001's boot router decides between Profile Setup and Home on this, and there was nothing to ask — both apps read `iam.users.first_name` off `GET /v1/users/me` instead. That is the **passenger** app's question: Profile Setup writes `registry.driver_profiles` and never touches `iam.users` (registry-svc holds no reference to that table and emits no event iam-svc consumes), so a driver who **had** completed it read as incomplete and was sent back to the form on every cold start, and a **passenger** who already had a name from the other app read as complete, skipped driver onboarding and reached Home without ever submitting a driving licence. **`null` is a 200, not a 404** — a cold-start read, same shape and same reason as `getActiveDriverRide`. The profile **row** only; the AL-29 extracted fields belong to the licence documents and come back from the PUT |
| GET `/v1/vehicles/{id}` | Bearer | [KEEP] | `VehicleDetail` (+ driver, status) | — |
| GET `/v1/drivers/{driverId}/profile-photo` | **Signed link** | [NEW] | image bytes, or `302` to a presigned URL | **Δ MCS-25.** The avatar both driver headers draw. `photoUrl` used to be `registry.driver_profiles.photo_url` verbatim, which is an `s3://` or `file://` pointer — this service's to resolve and not something a client can fetch — so a field typed `format: uri` carried a scheme no image loader understands and both apps drew a placeholder. **No bearer, deliberately:** the caller is an image loader and does not carry one, and a token in a query string is a token in every proxy log on the way. The signature names the driver and the photo; every refusal is the same `403` so a link nobody can use reveals no driver ids |
| GET `/v1/drivers/documents` | Bearer (driver) | [NEW] | `{items:[DriverDocument]}` | **Δ MCS-28.** The driver's own documents — their driving licence, and the four per vehicle they onboarded — each with a **signed image link**. `VehicleDocument` has carried `docId`, `kind`, `status` and `expiresAt` since C029 and **no URL at all**, so SCR-DA/DI-026 could tell a driver their insurance expired in March and could not show it to them. Driver-scoped in its own SQL: a document belonging to somebody else is not a `403`, it is absent, because telling the two apart leaks whose vehicle a document id names |
| GET `/v1/drivers/documents/{documentId}/image` | **Signed link** | [NEW] | image bytes, or `302` to a presigned URL | **Δ MCS-28.** The bytes behind one of those links, on the same arrangement as the profile photo above and for the same reason. The signature names the document AND the driver it belongs to, so a link cannot be edited into another driver's licence, and the handler re-checks ownership rather than trusting the signature alone — a signing key that ever leaked would otherwise be a key to every NIC on the platform |
...
| PUT `/v1/vehicles/{id}/onboarding/{step}` | Bearer (driver) | [NEW] | step ∈ {details,insurance,revenue,photos}; body = step fields/file ids → `{stepStatus: VERIFIED\|PENDING_REVIEW, onboardingStatus, nextStep}` | **Saves one onboarding step** (SCR-DA/DI-004→004c); each save persists `registry.onboarding_steps`; resume opens `nextStep`. **Δ MCS-01: the multipart arm is implemented** — `file`/`fileBack` plus a per-part `…CapturedVia` land in `docs.uploads` in the same request as the step's fields (it was declared from the start and bound JSON only). **Δ MCS-02: the multipart arm carries driver corrections** (`insuranceExpiry`, `insurancePolicyNo`, `revenueNo`, `revenueExpiry`) and `file` is **not** required when one is sent — SCR-DA/DI-004a/004b draw a ✎ on every extracted value and BR-25.3 is built on the driver editing one, but until then a correction had nowhere to go: this arm had no field for it and the JSON arm needs a `fileId` no response returns. A correction applies to the document already on record; the step must own one. Plate OCR ≠ reg-no (photos) or doubtful/manual field → step `PENDING_REVIEW` (AL-30, US-2.26/2.27). When the vehicle is already `approved`, a fresh `POST /v1/vehicles` starts a NEW vehicle at Step 1/4 |
| GET `/v1/vehicles/mine` | Bearer | [NEW] | `{items:[VehicleSummary], assigned:[VehicleSummary]}` | multi-vehicle (US-2.8). **Δ MCS-02: `VehicleSummary` gains `fleetName` + `assignedUntil`** — SCR-DA/DI-026's "Temporarily assigned to me" group prints *"Lanka Fleet (Pvt) Ltd · until 30 Jun"* (US-13.9) and the schema could name neither; `fleetId` is an identifier, not a caption. Both absent on an owned vehicle. Display only: the eligibility view stops returning an assignment the moment its window closes (migration 0314), so no client checks the date |
