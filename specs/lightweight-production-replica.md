...
| **`notification-svc`** | Push notifications via **FCM HTTP v1 batch send** + **APNs HTTP/2** with exponential-backoff worker. **Dispatch-offer push uses high-priority paths** (E-01): Android `priority=high` (bypasses Doze), iOS `apns-priority:10` + `content-available:1`; 3 s ack-wait → SMS fallback. SMS via primary (Notify.lk) + secondary (Dialog/Mobitel) for SOS. Renders templates from `content-svc` in user's language (Si/Ta/En). **SOS p99 ≤ 5 s** (D-33). **Proxy-booking location request** FCM (P-02, P-12): per-booker rate-limit 5/h, 30/day. **Package recipient notify** (P-09): FCM or SMS with trip-share-token link. Document-expiry warnings (E-03) | Separate stateless pod |
