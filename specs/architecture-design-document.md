...
- **Phone OTP via SMS gateway for user auth.** Pragmatic for MVP — phone-verified identity with low SMS cost via a local gateway (Fit SMS — AL-60). Firebase Auth retained as optional Google Sign-In bridge.
...
| D-11 | ~~OnePay driver-merchant onboarding missing~~ **RETIRED by AL-57** | OnePay supports one merchant account per merchant; per-driver sub-accounts do not exist, so the onboarding step, `registry.driver_payouts` and `POST /v1/internal/vehicles/{id}/merchant` are removed. Card ride payment is the passenger-wallet rail (AL-57) and the driver is paid by the AL-58 payout run | §1.18, §11.9 (**replaced**), §11.9a |
...
| D-25 | VoIP fallback undefined | ~~If VoIP fails, masked-number SMS relay via `notification-svc`; no direct number exposure~~ — **withdrawn by AL-48 (§1.13)**: VoIP failure now prompts **"Call normally instead?"** and the client places a direct `tel:` dial to the counterparty's real number | §6 `voip-svc`, §1.13 AL-48 |
...
| P-09 | Recipient must be reachable without exposing the **sender's** phone | FCM data-message to recipient (if registered) or SMS fallback with web-share token (if unregistered); the recipient sees a scoped token view, never the sender's number (`senderNameMasked`). `safety.trip_share_tokens` reused, scoped to package recipient role. *(The former "same masked-number policy as VoIP" clause lapsed with AL-48 — the driver's number IS shown to the recipient as a `tel:` link, US-26.3; the sender's is not.)* | §6 `notification-svc`, §6 `safety-svc` |
...
| ID | Deficit | Resolution | Section |
|---|---|---|---|
| T-01 | TCP/UDP adapter mentioned only as Phase 2 | **`tcp-adapter`** *(a.k.a. `tracker-adapter-svc` — alias only)* promoted to Phase 1; one StatefulSet *per protocol family* (GT06, JT/T 808, H02, generic-NMEA), normalising into MQTT | §6, §7.7, §10 |
...
| ID | Change request | Architecture resolution | Section / mapping |
|---|---|---|---|
| AL-36 | Onboarding "Get Started" CTA floated mid-screen; schedule-ride had no destination | **Client/UX:** passenger onboarding (SCR-PA-002 / SCR-PI-002) **pins Get Started to the bottom** below the language list. **Schedule-ride (SCR-PA-013 / SCR-PI-013) now requires a destination** — the same place-search/map-pick picker (`query-svc`/`transit-svc`, AL-17) supplies `dest_lat/lng`; pickup defaults to current GPS but is editable. `dispatch.scheduled_rides` already carries origin/destination — the API now **rejects a schedule create without a destination** (`dest_*` NOT NULL at the service boundary). *(Corrected 2026-07-26: this row originally read `scheduling.scheduled_rides` / `scheduling-svc`; scheduled rides are owned by **`dispatch-svc`** over **`dispatch.scheduled_rides`** — there is no `scheduling` service or schema anywhere in §6/§9.1.)* (URD US-24.1/24.2) | client, §6 `dispatch-svc`/`query-svc`, §9.1 `dispatch` |
...
### 1.18 Remediation Log (ADD v3.6 — Payment-custody change set 2026-08-01, 3 changes)

**Trigger — a vendor fact that invalidates D-11.** OnePay supports **one merchant account per merchant**; it does not create per-driver merchant sub-accounts. D-11 ("OnePay driver-merchant onboarding", §11.9) assumed it did, and every mechanism built on that assumption — `registry.driver_payouts.onepay_merchant_id`, `POST /v1/internal/vehicles/{id}/merchant`, `Onepay__MerchantOnboardingUrl`, `402 merchant-not-onboarded` — can only ever hold or check MageRide's own merchant id, the same value for every driver. D-11 is therefore **retired, not completed**.

That leaves one question: on a card payment, **whose account does a ride fare land in?** With a single merchant account the answer is MageRide's. The platform's previous posture (AL-01 no commission, AL-05 no bank rail, AL-49 "MageRide still holds no subscriber money") was pass-through throughout, so this is a deliberate reversal: **MageRide now takes custody of card-paid ride fares and owes the driver a payout.** Card acceptance is retained because cash-only in-app payment was judged unacceptable; the custody and the payout obligation are its price.

> ⚠ **Licensing precondition.** Holding passenger prepaid balances and executing outward bank transfers makes MageRide a payment intermediary rather than a pass-through, which in Sri Lanka engages the Payment and Settlement Systems Act (CBSL authorisation) and requires a sponsor bank for LankaPay/CEFTS origination. This is a **go-live gate**, not an engineering task, and no component below discharges it.

| ID | Change request | Architecture resolution | Section / mapping |
|---|---|---|---|
| AL-57 | **D-11's per-driver OnePay merchant does not exist** (one account per merchant), so a card ride fare has nowhere to land except MageRide's own account — and `402 merchant-not-onboarded` blocks the rail outright | **D-11 retired; card ride payments routed through a passenger wallet.** OnePay's single merchant account is used **only where MageRide is the payee** — passenger wallet top-ups, driver daily fees, bulk-voucher purchases. `onepay` is **removed as a `fares.ride_payments.method`**; a passenger who wants to pay by card **tops up their MageRide wallet** (OnePay, MageRide legitimately the payee) and pays the ride with the new method **`wallet`**. A wallet-paid fare is one balanced `trip_payment` journal entry — passenger wallet debit, driver wallet credit — settled **atomically at payment time with no `Pending` state and no gateway callback**, because no gateway is involved. `billing.accounts.owner_type` gains **`passenger`** (it carried `driver`/`fleet`/`platform`/`suspense` only, so a passenger wallet had no ledger existence). **Removed:** `registry.driver_payouts`, `POST /v1/internal/vehicles/{id}/merchant`, `Onepay__MerchantOnboardingUrl`, error code `merchant-not-onboarded`, §11.9 | §6 `fare-svc`/`wallet-svc`/`registry-svc`, §9.1 `billing`/`registry`, §11.9 (**retired**), §11.14; D3 fare/wallet/registry route tables, D4 §10/§2, D5 §8.1, D7 §4.2 |
| AL-58 | Custody creates an obligation the platform has no way to discharge: a driver's wallet balance is now **money MageRide owes**, and AL-05 deleted the only outward bank rail | **Automated driver payout rail.** New **`registry.driver_payout_profiles`** — bank, branch, account number, account holder name, proof-of-account upload, and the driver's own bank-app LankaQR image; **versioned with one `verified` row per driver** and approved by a **Verification Officer through the AL-39 queue family** — a **fourth SCR-AP-003 tab** (`GET /v1/admin/verification/queues/driver-payout`) and its own detail and verdict routes (`/v1/admin/verification/payout/{driverId}[/approve|/reject]`), forwarded to registry-svc's `/v1/internal/drivers/{driverId}/payout-profile/**`. **Corrected 2026-08-01 (C063):** this originally read "whose subject-agnostic routes already take a driver id", which does not survive contact with two facts. (a) The existing queues are "has a flagged `registry.document_fields` row" and nothing extracts fields from a bank statement, so a pending payout profile appeared in **no queue at all** — a driver who changes banks after approval was invisible and was never paid. (b) A driver id already resolves to the **identity** subject, so sharing `POST /v1/admin/verification/{subjectId}/approve` would make one button decide two unrelated questions, and a refusal aimed at an illegible bank statement would refuse the driver's **licence** and stop them driving. The officer, the RBAC row, the audited document viewer and the screen are shared; the predicate, the verdict route and the audit action (`PAYOUT_PROFILE_APPROVED`) are not. New **`billing.payout_batches`** + **`billing.payouts`**, a scheduled settlement run that debits the driver wallet and raises a payout instruction under the new journal kind **`driver_payout`**, and a **bank-origination seam** (`Payout__*`) which, when unconfigured, leaves payouts `PENDING` and announces it at start-up rather than silently accruing. A driver with **no verified payout profile accrues normally and is never paid out** — the balance is retained, never lost, and appears on the Finance exception queue (SCR-AP-006). Payout execution is **provider-agnostic by construction**: the run produces instructions; the adapter that submits them is a single outbound port | §6 **new `payout-svc`**/`wallet-svc`, §9.1 `billing`/`registry`, §11.9a (new); D2 SCR-DA-022a (new), SCR-AP-006; D3 wallet/registry/admin-bff; D4 §10/§2; D7 §4.2 |
| AL-59 | The `lankaqr` ride rail pointed at **MageRide's** merchant (`LankaQr__MerchantId`), so it collected fares into the platform account while crediting the driver only a `fares.driver_earnings` read-model row — a second, undetected custody path — and `onepay` was offered for Mode B subscription payments whose `payTo` is the **fleet owner's** account (AL-49) | **A LankaQR ride payment is the driver's own QR, always.** Drivers hold their own bank-issued LankaQR (AL-22); the image lives on the AL-58 payout profile and is served to the passenger as a signed URL, exactly as AL-49 serves a fleet owner's. The platform-merchant `lankaqr` ride method is **retired** and collapses into **`scan_driver_qr`**, which already settles by AL-47 attestation precisely because a payment into somebody else's bank produces no platform webhook. `LankaQr__MerchantId` survives **only** for wallet top-ups, where MageRide is the payee. **`onepay` removed from Mode B subscription payment methods** — it would have landed subscriber money in MageRide's account against AL-49's pass-through rule; `lankaqr_scan`, `lankaqr_deeplink`, `online_transfer` and `cash` all already pay the owner directly | §6 `fare-svc`/`subscription-svc`, §9.1 `fares`; D3 fare/subscription route tables, D5 §8.1, D7 §4.2 |
...
platform implemented it as one, in two services: iam-svc's OTP sender and notification-svc's
gateway. The platform's SMS now runs on **Fit SMS** (`https://app.fitsms.lk/api/v4/`).

The question that decides the shape of the change: **is Notify.lk kept as a switchable
alternative?** No. A gateway nobody holds credentials for is a code path no deployment exercises
and no test can honestly cover — it reads as a fallback and is not one. The client is removed, and
a deployment still naming `notifylk` is **refused at start-up** rather than falling through to the
dev sender, which would log every OTP and let nobody sign in while looking healthy.

**What is NOT changed:** §7.3's **secondary** gateway. That half is a generic HTTP shape rather
than a named provider, and D-33 still sends an SOS through both transports in parallel.

| ID | Change request | Architecture resolution | Section / mapping |
|---|---|---|---|
| AL-60 | The platform's SMS account moved from Notify.lk to Fit SMS. Notify.lk was the named primary in D6' §7.3 and was implemented in both iam-svc (OTP) and notification-svc (every other SMS) | **Fit SMS is the platform's only SMS gateway.** v4 REST, one bearer token issued as `{id}\|{secret}`; `POST sms/send {recipient, sender_id, type, message, expiry_time}`. Three properties are load-bearing and each is a way to lose every message silently: **(1)** a refused send answers **HTTP 200 with `{"status":"error"}`**, so the status line is not the outcome and the body must be read; **(2)** `sender_id` must be a mask Fit SMS has **approved on the account** — an unapproved one is refused with `Sender ID "X" is not authorized to send this message`, under that same 200; **(3)** `type` must be `unicode` for a non-ASCII body — AL-26 makes Sinhala the default language, so the common message on this platform is UCS-2 and sending it as `plain` delivers question marks. `data.ruid` is the id their `GET sms/{ruid}` traces an undelivered message by. **Notify.lk's client, options and Vault properties are removed**; `Sms__Provider` accepts `fitsms` or `dev` and refuses anything else. §7.3's secondary gateway is unchanged | D6' §7.3; D3' `/auth/otp/request`; D5' OTP row; D7' §4.2 (`Sms__FitSmsApiToken`, `Sms__FitSmsSenderId`, `Sms__FitSmsUnicodeType`; `Sms__NotifyLk*` removed); §6 `iam-svc` / `notification-svc`; replica spec Container 7 |
...
| **Notifications** | FCM push, SMS via gateway (Fit SMS — AL-60), interest alerts, access request alerts, **low-balance / daily fee warnings**, session auto-end alerts, **dispatch offers**, **scheduled-ride reminders**, **payment confirmations**, **SOS** | Stateless + Postgres `notifications` schema (preferences) |
...
| `wallet-svc` | .NET 10 LTS + **OnePay** + **LankaQR** | **Passenger wallet** (AL-57 — a card top-up is held as a prepaid balance and a `wallet` ride payment is one balanced `trip_payment` entry, passenger wallet → driver wallet, terminal on the spot with no gateway leg) and **driver wallet** balance management (one wallet per driver — a reselling driver uses the same wallet, **no separate reseller account**, AL-01) on a **double-entry ledger** (`billing.accounts`, `billing.journal_entries`, `billing.journal_postings` — balanced postings idempotent on `Idempotency-Key`). **In-app top-ups via OnePay card / OnePay wallet / LankaQR only — bank transfer removed (AL-05)**; OnePay/LankaQR **gateway settlement reconciliation** via Commercial Bank IPG webhook (exceptions → Finance queue in Admin Portal). **Bulk credit voucher** purchase (per-tier DB discount applied at purchase), balance tracking, and **driver-to-driver credit-transfer ledger (exact value, no commission)**. Publishes `wallet.debited` / `wallet.credited` events that invalidate `dispatch-svc` Redis balance cache | Stateless |
| `payout-svc` | .NET 10 LTS + **bank origination (provider TBD)** | **[NEW AL-58] The automated driver payout rail.** A scheduled run selects drivers with a `verified` `registry.driver_payout_profiles` row and a wallet balance at or above the configured minimum, debits the wallet under journal kind `driver_payout` and raises a `billing.payouts` instruction **in the same transaction** — a payout with no debit pays twice on a retry, a debit with no payout loses the driver's money. Instructions move `PENDING → SUBMITTED → PAID | FAILED`; `FAILED` reverses the debit under the same idempotency key. A driver with **no verified profile accrues and is never paid out** — the balance is retained and they appear on the Finance exception queue (SCR-AP-006). **The bank adapter is one outbound port and no provider is chosen**: unconfigured, the run still records what is owed and announces the gap at start-up, so an operator can see the liability before a rail exists. ⚠ Origination via LankaPay/CEFTS requires a sponsor bank and CBSL authorisation (§1.18) | Stateless + scheduled |
...
registry.driver_payout_profiles -- [NEW AL-58] driver bank & payout: bank, branch, account_no, account_holder_name, proof_upload_id, lankaqr_upload_id (the driver's own bank-app QR, AL-59); status ∈ {pending_verification,verified,rejected,superseded}; one verified row per driver (partial UQ); Verification-Officer approved through the AL-39 queue; the payout run pays the latest verified row and the ride pay sheet renders its QR
                           --   (replaces registry.driver_payouts, retired with D-11 — AL-57)
...
billing.accounts                 -- (id, owner_type ∈ {passenger,driver,fleet,platform,suspense}, owner_id, currency, balance_minor) — no 'reseller' owner_type (AL-01); 'fleet' added for fleet wallet (AL-03); 'passenger' added for the AL-57 card rail, where a top-up is held as a prepaid balance and a ride is paid from it
billing.journal_entries          -- (id, ts, kind ∈ {topup,voucher_purchase,daily_fee,trip_payment,penalty_settle,credit_transfer,adjustment,tip_payout,payment_refund,overpaid_reversal,fleet_invoice,driver_payout}, idempotency_key UNIQUE, description)
                                 --   'driver_payout' [NEW AL-58] discharges the liability an AL-57 wallet fare created: driver wallet debit against the platform account, keyed 'driver_payout:' || payout_id
billing.payout_batches           -- [NEW AL-58] one scheduled payout run (id, started_at, completed_at, status, instruction_count, total_minor)
billing.payouts                  -- [NEW AL-58] one instruction per driver per batch (id, batch_id, driver_id, payout_profile_id, amount_minor, status ∈ {PENDING,SUBMITTED,PAID,FAILED}, failure_reason, provider_reference UNIQUE NULLS DISTINCT, journal_entry_id, ts) — the wallet debit and this row commit together; FAILED reverses the debit under the same key discipline
...
### 11.9 Card Ride Payment via Passenger Wallet (AL-57) — supersedes OnePay Driver-Merchant Onboarding (D-11, retired)

**D-11 is retired.** It assumed OnePay would mint a merchant sub-account per driver so a card fare
could land in the driver's own account. OnePay supports **one merchant account per merchant**, so
that mechanism never existed. `registry.driver_payouts`, `POST /v1/internal/vehicles/{id}/merchant`
and `Onepay__MerchantOnboardingUrl` are removed; nothing replaces them in registry-svc.

Card payment is retained by moving it one step earlier, to a rail where **MageRide is legitimately
the payee**: the passenger's own wallet.

```
Passenger      wallet-svc        OnePay          fare-svc        Driver wallet
   │  top up       │               │                │                 │
   │  (card)       │               │                │                 │
   ├──────────────►│  charge       │                │                 │
   │               ├──────────────►│                │                 │
   ...
   │               │  journal: topup                │                 │
   │               │  platform → passenger wallet   │                 │
   │◄──────────────│               │                │                 │
   │                                                │                 │
   │  pay ride, method = 'wallet'                   │                 │
   ├───────────────────────────────────────────────►│                 │
   │                                │  journal: trip_payment          │
   │                                │  passenger wallet ──────────────►
   │                                │  (one balanced entry, terminal   │
   │                                │   on the spot — no gateway,      │
   │                                │   no Pending, no callback)       │
   │◄───────────────────────────────│                                 │
```

Three consequences, each load-bearing:

- **`wallet` has no `Pending` state.** Every other rail waits on an acquirer; this one is a ledger
  move inside one transaction, so it reaches `Succeeded` or it does not happen. D5 §8.1's machine
  gains the transition and no callback path.
- **An insufficient balance is refused before the ride is settled**, not fallen back to cash
  silently — the passenger chose a rail and must be told it is short, with cash and driver-QR still
  offered.
- **The driver's wallet balance is now a liability, not a float.** Before AL-57 it held prepaid
  credit the driver had paid *in* against daily fees; it now also accumulates fares MageRide has
  collected on their behalf. Discharging that is §11.9a.

### 11.9a Automated Driver Payout Run (AL-58)

```
scheduler        payout-svc            wallet-svc         bank origination
    │  interval      │                     │                  │
    ├───────────────►│  select drivers with a VERIFIED         │
    │                │  payout profile and balance ≥ minimum   │
    │                ├────────────────────►│  debit wallet,    │
    │                │                     │  journal kind     │
    │                │                     │  'driver_payout'  │
    │                │◄────────────────────│                   │
    │                │  billing.payouts row = PENDING          │
    │                ├────────────────────────────────────────►│  submit instruction
    │                │                                         │  (CEFTS / sponsor bank)
    │                │◄────────────────────────────────────────│  accepted | rejected
...
- **The debit and the payout row commit together.** A payout instruction with no matching debit pays
  a driver twice on a retry; a debit with no instruction loses their money.
- **A driver with no `verified` payout profile is never paid out, and never loses the balance.** The
  run skips them and the amount stays on the wallet; Finance sees them on SCR-AP-006's exception
  queue. Approval is the Verification Officer's, through the **existing AL-39 queue** — its routes
  are subject-agnostic and already take a driver id.
- **`FAILED` returns the money.** A rejected instruction reverses the debit under the same
  idempotency key discipline as every other ledger movement (§0), so the balance is restored exactly
  once and the next run retries it.
- **The bank adapter is one outbound port, and the platform has no provider chosen.** Unconfigured,
  the run still selects and still records `PENDING` rows — it simply submits nothing, and says so at
  start-up. That is deliberate: an operator can see what is owed before a rail exists.
...
|---|---|---|
| End users (passengers) | Phone OTP via SMS gateway (Fit SMS — AL-60) — primary; **OTP send rate-limited (Redis token bucket: 60 s resend, 5/h) (D-32)** | JWT RS256, 30 min access / 30d refresh |
...
| Account takeover | A07 | Phone-OTP sign-in for drivers (SMS OTP with 60 s resend cooldown, 5/h cap), **Android Keystore** / **iOS Keychain + Secure Enclave** device binding, Play Integrity (Android) / **App Attest** (iOS). Internal roles: failed-attempt lock-out + session binding + optional IP allow-list (**no second factor — AL-37**) |
...
| Auth | Phone OTP (SMS gateway — Fit SMS, AL-60) + Firebase Auth (Google Sign-In optional) | Keycloak at scale |
...
| Payment (ride fares) | **Cash** (default) + **scan-driver-QR** (the driver's own bank LankaQR, no surcharge; settles by **attestation → `DriverConfirmedQR`**, AL-47/AL-59) + **`wallet`** (AL-57 — prepaid balance topped up by card; one balanced `trip_payment` entry, terminal on the spot) + **COD** (packages) with **persisted payment state machine** (Initiated→Pending→Succeeded/Failed/Retried/FellBackToCash/QrClaimedByPassenger/DriverConfirmedQR) | PayHere | **AL-57/AL-59: no ride fare is charged to a platform merchant account.** `onepay` and platform-merchant `lankaqr` are removed as ride methods — OnePay collects only where MageRide is the payee (wallet top-ups, daily fees, vouchers) and its ~3% fee is recovered on the top-up. Driver-QR is bank-to-bank into the driver's own account: no platform webhook, no bank IPG (AL-05/AL-47). A `wallet` fare has no gateway leg at all; MageRide holds the balance and discharges it through the AL-58 payout run |
...
| **Firebase Phone/OTP auth** | **$0.27/SMS in Sri Lanka with no free allowance.** Use a **local SMS gateway** (Fit SMS — AL-60) (was e.g. Notify.lk at Rs 0.50–1.50/SMS) for OTP delivery instead |
...
- [ ] **Hardware GPS Tracker plane (promoted to Phase 1, v2.3 §1.6, T-01..T-12)** — `provisioning-svc` (step-ca + Vault PKI, per-device X.509 / HMAC, 90-day rotation, anti-clone), `tcp-adapter` per protocol family (GT06, JT808, H02, NMEA-UDP), `fleet-health-svc`, TimescaleDB hypertable `telemetry.positions` (continuous aggregates + 7-day compression) — URD Epic 3 (NFR-34..NFR-44)
...
- [x] ~~**`registry-svc` OnePay merchant onboarding**~~ — **D-11 retired by AL-57** (no per-driver OnePay merchant exists)
- [ ] **`registry-svc` driver payout profile** (bank + own LankaQR, Verification-Officer approved via the AL-39 queue) — AL-58/AL-59
- [ ] **`payout-svc` automated driver payout run** (wallet debit → `billing.payouts` instruction → bank origination seam) — AL-58
...
POST   /fare/pay                   # initiate payment (method: cash | wallet | scan_driver_qr | cod)   [AL-57/AL-59: onepay and platform lankaqr removed]
