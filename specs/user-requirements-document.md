...
| US-2.27 | **＋ in My Vehicles always starts a fresh Step 1/4 for a NEW vehicle**, whatever else is unfinished — ＋ means add. Continuing an **Incomplete** vehicle is the row's own **Resume ›**, which opens **that** vehicle at its next incomplete step; so does SCR-DA/DI-006's *Continue*. Opening **Vehicle Onboarding from the nav drawer**, which names no vehicle, resumes the first Incomplete vehicle and starts a fresh Step 1/4 only once every vehicle is **Approved**. *(2026-06-25 item 6; ＋ / Resume split by MCS-06, 2026-08-21 — ＋ previously resumed too, which put a driver adding a second vehicle on "Step 2 of 4 · Insurance" for their first)* | P0 |
...
| ID | Story | Priority |
|---|---|---|
| US-11.1 | The platform has **five surfaces**: MageRide Passenger App (Android & IOS), MageRide Driver App (Android & IOS), the **Admin Portal** (`admin.mageride.lk`, back-office web application for all internal roles), the **Fleet Portal** (`fleet.mageride.lk`, responsive web application for fleet owners), and the **public informational site** (`www.mageride.lk`, the platform's marketing and how-to-use site — **public, unauthenticated, carrying no personal data and with no API dependency at request time**; it renders with the backend down, and the `mageride.lk` apex 301-redirects to it — **Δ 2026-08-28 (MCS-34)**). `passenger.mageride.lk` is **not a separate surface** — it is a **no-login web subview of the Passenger App** (US-11.9), opened only via a tokenised SMS link when a driver accepts a proxy ride (US-8.22) | P0 |
...
| US-11.8 | The **four product surfaces** (Passenger App, Driver App, Admin Portal, Fleet Portal) share the same backend platform and identity service; the Fleet Portal's email/Google/Apple identities, the Driver/Passenger Phone-OTP identities, and the Admin Portal's password/Google identities resolve to a single unified account model. **`www.mageride.lk` is outside this clause** — it authenticates nobody, holds no account model and calls no API at request time (**Δ 2026-08-28 (MCS-34)**) | P0 |
...
| Term | Definition |
|---|---|
| **MageRide** | The platform brand name. Has **five surfaces**: MageRide Passenger App, MageRide Driver App, the Admin Portal (`admin.mageride.lk`), the Fleet Portal (`fleet.mageride.lk`), and the public informational site (`www.mageride.lk` — public, unauthenticated, no personal data, no API dependency at request time; **Δ 2026-08-28 (MCS-34)**). `passenger.mageride.lk` is a no-login **web subview of the Passenger App**, not a separate surface. |
