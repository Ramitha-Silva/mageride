...
import { createAdminTranslator, DEFAULT_LOCALE, isLocale, negotiateLocale, type AdminTranslator, type Locale } from './index';
 ...
 *  2. the operator's **stored account language**, which is what they picked on
 ...
 *     back-office should not become English-first for being in a browser.
 *
 ...
 * bookmark, every deep link an operator pastes into a ticket, and every `next=`
 * redirect carry a language somebody else then reads the page in.
 *
 * ## Appearance
 *
...
export const getTranslator = cache(async (): Promise<AdminTranslator> => {
  return createAdminTranslator(await getLocale());
});
