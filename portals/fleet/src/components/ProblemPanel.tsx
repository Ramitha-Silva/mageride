...
/**
 * How an `application/problem+json` failure is put in front of an operator.
 *
...
export async function ProblemPanel({ problem }: { problem: ProblemDetails }) {
  ...
  return (
    ...
    >
      <p className="text-body-sm font-semibold text-error">{t('fleet.error.title')}</p>
      <p className="text-body-sm text-on-surface">{t(problemMessageKey(problem))}</p>
      {problem.traceId ? (
        <p className="text-caption text-on-surface-variant">
          {t('fleet.error.reference', { traceId: problem.traceId })}
        </p>
      ) : null}
    </div>
  );
