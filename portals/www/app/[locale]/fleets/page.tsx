...
/**
 * `/{locale}/fleets` — one of the four role landing pages.
 *
...
export async function generateMetadata({ params }: { params: Promise<LocaleParams> }) {
  return metadataForRoute(await localeFrom(params), 'fleets');
}

export default async function FleetsPage({ params }: { params: Promise<LocaleParams> }) {
  return <RolePage locale={await localeFrom(params)} path="fleets" />;
}
