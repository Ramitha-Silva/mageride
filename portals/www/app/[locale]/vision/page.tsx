...
/**
 * `/{locale}/vision` — one of the four role landing pages.
 *
...
export async function generateMetadata({ params }: { params: Promise<LocaleParams> }) {
  return metadataForRoute(await localeFrom(params), 'vision');
}

export default async function VisionPage({ params }: { params: Promise<LocaleParams> }) {
  return <RolePage locale={await localeFrom(params)} path="vision" />;
}
