...
/**
 * The fleet-guide chapter card — S19's fourth family, extended to the third guide
 * by S23.
 *
 * **The title is read from the chapter registry**, in English, through the same
 * translator the page uses. `src/components/seo/OgCard.tsx` records why the cards are
 * English and it is a font-availability finding rather than a choice; what matters
 * here is that the string is still *derived* — a chapter renamed in the corpus renames
 * its card, and no card can name a chapter that does not exist.
 *
 * **Prerendered, one per chapter per rendered locale.** Rendering these on demand
 * would put satori and a WASM rasteriser on the path every preview bot walks, on the
 * surface whose defining property is having no request-time work at all.
 */
export const size = OG_SIZE;
export const contentType = OG_CONTENT_TYPE;
const t = createWwwTranslator(FALLBACK_LOCALE);

export const alt = `${t('www.brand.name')} — ${t('www.page.guide.fleetHeading')}`;
  ...
  return new ImageResponse(
    (
      <OgCard
        ...
        eyebrow={t('www.page.guide.fleetHeading')}
