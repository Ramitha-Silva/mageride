...
///   backup, so a restored backup cannot resume a session on another handset; `AfterFirstUnlock`
///   rather than `WhenUnlocked` because a driver's phone is locked in a mount for most of a ride and
///   the MQTT renewal loop must be able to read its credential then), and `kSecAttrService`,
...
///   Keychain, bound into the session at OTP verify and into every MQTT token (E-02). It is
///   deliberately not `identifierForVendor`, which resets when the last app from a vendor is
///   deleted — a driver who reinstalled would look like a new device and AL-08 would revoke the
///   session they still had.
...
/// all, and what to do about the registration that has no endpoint.
enum DeviceBinding {
    ...
    /// build can produce assertions the edge cannot check, so D-30's twenty operations answer
    /// `401 attestation-failed` on iOS.
    ///
    /// The method is here rather than deleted because the *client* half is complete and the gap is
    /// one endpoint wide: when the route lands, C086 calls this and posts the result. Deleting it
    /// would hide a finished half behind an unfinished one.
