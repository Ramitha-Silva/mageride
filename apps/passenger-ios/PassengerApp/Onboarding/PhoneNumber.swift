...
/// with the trunk zero, `771234567` without) both have to work: a passenger reading their number off
/// a bill will type the first.
///
/// The same four steps in the same order as
/// `apps/passenger-android/.../onboarding/PhoneNumber.kt` — and as the driver app's, on both
/// platforms. A number that is valid on one surface and not another is a passenger who cannot sign
/// in on their second handset.
enum PhoneNumber {
    ...
    /// validator would reject — which is what makes a pasted `+94 77 123 4567` simply work, and what
    /// makes the CTA's enabled state a property of the field rather than a check at submit time.
    static func normalise(_ input: String) -> String {
        ...
        // built out of this, and `+94෧෭…` is not a phone number the gateway can dial — a Sinhala
        // keyboard that produces another script's digits should fail the field, not the request.
