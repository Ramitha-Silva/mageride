...
/// 2. **``PassengerLocale`` redirects that bundle** when a passenger picks a language, so every
///    lookup that goes through it follows SCR-PI-002's answer and one that does not stays in the
///    handset's language. There is no third way to look a string up in this target.
extension String {

    /// This key, resolved in the passenger's language.
...
extension Text {
    ...
    /// A `Text` for a key in the passenger's language.
    ///
