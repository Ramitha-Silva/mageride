...
// a bare `Font.system(size: 32)` honours the size and does not scale, so a passenger who has
// turned text up reads the same small label they could not read before — and US-19.1/19.2 and
// D2' §C ("VoiceOver + Dynamic Type") are the requirement, not a nicety.
//
...
//
// **Identical in substance to `apps/driver-ios/DriverApp/Theme/MageRideTypography.swift`** — the
// eight rows, the pair of size and curve, and the one modifier. The C085 handoff
// asked C094 to promote the shared theme files into a Swift package rather than make a third copy;
// the C094 handoff argues why that was deferred, and this file is the strongest case for doing it —
// there is nothing app-specific in it at all.
//
...
// sizes and weights. That is the same real deviation from §0.2 that C067 and C085 both recorded,
// and it closes the same way — a licensed face, which is a design-procurement decision.
...
extension View {
    ...
    /// or ignoring the passenger's Dynamic Type setting.
