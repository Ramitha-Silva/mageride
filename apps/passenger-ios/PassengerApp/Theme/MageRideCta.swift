...
/// `Button(…) { }.buttonStyle(.mageCta)` and the token is applied. (Compose has no `ButtonStyle`,
/// which is why the Android twin needed `MageRideCta` and `MageRideCtaTonal` as two *views*.)
///
...
/// submit waiting to happen, and on this surface a double submit is a second ride.
struct MageRideCtaStyle: ButtonStyle {

    /// Which of §0.2's fills. `tonal` is the wireframe's `.cta.tonal`, `destructive` its `.cta.err`
    /// (SCR-PA-015's *"Cancel ride"*, SCR-PA-025's *"Unsubscribe"*).
    ///
    /// The driver app carries a fourth case — a role colour its Start/End Journey pair fixes — and
    /// it is deliberately absent here: no passenger wireframe cell fixes a CTA colour by
    /// consequence. A screen group that finds one adds the case rather than a second control.
    enum Emphasis {
        case filled
        case tonal
        case destructive
...
extension ButtonStyle where Self == MageRideCtaStyle {
    ...
    /// The `error` fill — the wireframe's `.cta.err`.
