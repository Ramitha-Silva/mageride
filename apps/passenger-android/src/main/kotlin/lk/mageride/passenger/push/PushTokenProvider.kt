 ...
 *
 * `POST /v1/auth/otp/request` carries it (`RequestOtpRequest.fcmToken`) so a package addressed to
 * a brand-new passenger has somewhere to land before they have opened the app a second time.
 *
 ...
 * Failing a passenger's login over a push token that is inert anyway would be the wrong trade, so
