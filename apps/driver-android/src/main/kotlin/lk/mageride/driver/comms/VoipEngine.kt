 ...
 *
 * Three states and no more, because SCR-DA-031 draws three: *"Connecting…"*, *"Connected · 00:42"*
 * and the failure that raises AL-48's *"Call normally instead?"*. Hanging up is not a state the
 * engine reports — it is the driver leaving, which the view model knows before the engine does.
 */
 ...
 * column has one value for "the free call did not happen" and a `direct_dial` row after one is the
 * fallback being taken (see `VoipApi.recordCallOutcome`). The distinction is for the driver-facing
 * copy, which differs: a ride that has ended has nobody left to call, and telling the driver to
 * dial normally would be wrong advice.
 */
internal enum class VoipFailure {
     ...
     * SCR-DA-031 currently reaches.
     */
    NO_MEDIA_CLIENT,
}

/**
 * The WebRTC half of SCR-DA-031 — D6' §6's LiveKit room, behind an interface.
 *
 * A seam rather than a direct SDK call for the reason `PositionPublisher` and `PaymentHandoff` are:
 * everything above it — the call's state machine, its timer, its outcome reporting and AL-48's
 ...
 *
 * ### Why (Δ C075, and it is a dependency wall rather than a decision)
 *
 * D6' §6 names **LiveKit** as the SFU and D2' §SCR-DA-031's component note says *"Δ Android: WebRTC
 ...
 * widen. It is recorded as a gap in the C075 handoff.
 *
 * ### What that means on the screen, and why it is still the specified behaviour
 *
 * The **signalling half is real and complete**: `POST /v1/calls/start` is called with
 * `free_voip`, voip-svc mints the room token and writes `comms.call_log`, and this engine then
 * reports [VoipFailure.NO_MEDIA_CLIENT] — which is exactly the condition AL-48 legislates for.
 * SCR-DA-031 puts up *"Call normally instead?"* and the driver reaches the rider on a `tel:` dial
 * of the number the ride already carries. Nothing is faked and nothing is silently swallowed: the
 ...
 *
 * Replace this binding in `driverAppModule` and nothing else: add `RECORD_AUDIO` and
 * `MODIFY_AUDIO_SETTINGS` to the manifest (they are absent on purpose — a permission with no code
 * behind it is what that file's own header forbids), and emit [CallLink.Connected] from the room's
