 ...
 * is the assertion that keeps the Compose half of that promise honest.
 *
 * Every expectation below is typed from the spec's own table rather than read back out of the
 * production object, which is the only way a test of a constant is worth anything: if
 * `Color.kt` and the table disagree, exactly one of them is wrong and this says which.
 *
 * The same hexes are the iOS asset catalogue's (C085) and the Tailwind preset's (AL-52). A drift
 * here would show up as three surfaces that look almost the same, which is the hardest kind of
 * inconsistency to find in review.
 */
class ThemeTokensTest {
    ...
    fun the_map_03_vehicle_legend_matches_the_d2_0_2_table() {
        ...
        // confusion Mode A cannot afford.
